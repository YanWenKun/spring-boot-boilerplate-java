...
# 即使在容器中，也应当使用低特权用户
RUN mkdir -p /home/maven /build && \
        useradd maven -d /home/maven && \
        chown -R maven:maven /home/maven /build
USER maven:maven

# 配置 Maven 使用镜像源（阿里云）
COPY --chown=maven:maven docs/maven/settings.xml /home/maven/.m2/settings.xml
